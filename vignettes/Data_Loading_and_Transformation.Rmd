...
- `auto_logi`: For HDFC and Spectral Flow data (recommended, auto-logicle transformation). This transformation gives good results also with cyTOF data, expecially if you are experiencing a lot of noise with `arcsinh` due to negative values. auto-logicle transformation is inherited from the `Cytofkit` package [Chen er al. 2016](https://pubmed.ncbi.nlm.nih.gov/27662185/).
- `clr`: Reccomended for CITE-seq data (centered log ratio transformtation)
...
Unwanted parameters that are not important for the downstream analysis (e.g. Time, InFile) and should be removed can be listed in `remove_param`. In the `prep_fcd` function we also set a `seed` for reproducibility since the subsetting to `max_cell` is otherwise randomized.
