...
The calculation of the Principle Components is based on the `prcomp` function from the R Stats package (https://rdocumentation.org/packages/stats/versions/3.6.2). Alternatively you can use the GPU accelerated version of the PCA algorithm by setting the parameter `GPU=T` (only available in the respective docker image and supported hardware). For more details see: https://rapids-singlecell.readthedocs.io/en/latest/.
