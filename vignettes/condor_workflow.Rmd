...
condor <- metaclustering(fcd = condor, 
                         clustering = "Phenograph_pca_orig_k60", 
                         name_col = "Phenograph", 
...
```{r}
condor <- metaclustering(fcd = condor, 
                         clustering = "FlowSOM_pca_orig_k5", 
                         name_col = "FlowSOM", 
                         name_out = "metaclusters", 
                         metaclusters = c("CD4 T", 
                                          "NK cells", 
...
```{r}
plot_marker(data = cbind(condor$umap$pca_orig, condor$clustering$Phenograph_pca_orig_k60), 
            param = "Phenograph", 
            order = T, 
...
```{r}
plot_marker(data = cbind(condor$umap$pca_orig, condor$clustering$FlowSOM_pca_orig_k5), 
            param = "FlowSOM", 
            order = T, 
            title = "Phenograph Clustering on Normalized expression", 
            dim_red = "UMAP", 
...
plot_marker_2(data = cbind(condor$umap$pca_orig, condor$clustering$Phenograph_pca_orig_k60, condor$anno$cell_anno), 
            param = "metaclusters", 
            order = T, 
            title = "Phenograph Clustering on Normalized expression", 
...
```{r, fig.width=15}
plot_density(data = cbind(condor$umap$pca_orig, condor$clustering$Phenograph_pca_orig_k60, condor$anno$cell_anno), 
             param = "group", 
             title = "test", 
             dim_red = "UMAP", 
             dot_size = 0.2, 
...
```{r}
confusion_HM(variables = condor$clustering$Phenograph_pca_orig_k60$Phenograph, 
             group = condor$anno$cell_anno$group, size = 30, 
             title = "Confusion Matrix - Phenograph K60")
```

```{r}
confusion_HM(variables = condor$clustering$Phenograph_pca_orig_k60$metaclusters, 
             group = condor$anno$cell_anno$group, size = 30, 
...
barplot_frequency(x_axes = condor$anno$cell_anno$group, 
                  colour = condor$clustering$Phenograph_pca_orig_k60$Phenograph, 
...
```{r}
barplot_frequency(x_axes = condor$anno$cell_anno$group, 
                  colour = condor$clustering$Phenograph_pca_orig_k60$metaclusters, 
...
```{r, fig.height=7}
HM_markers(input = cbind(condor$expr$orig, condor$clustering$Phenograph_pca_orig_k60), 
           group = "Phenograph",
           maxvalue = 2, 
...
box_stat <- boxplot_and_stats(annotation = condor$anno$cell_anno, 
                              ...
                              group_var = "group", 
                              variable = condor$clustering$Phenograph_pca_orig_k60$metaclusters, 
                              numeric = FALSE, 
                              test.type = "t.test")
...
condor_filter <- filter_fcd(fcdataset = condor, 
                            cell_ids = rownames(condor$clustering$Phenograph_pca_orig_k60[condor$clustering$Phenograph_pca_orig_k60$metaclusters == "CD4 T", ]))
```

```{r}
plot_marker(data = cbind(condor_filter$umap$pca_norm, condor_filter$clustering$Phenograph_pca_orig_k60), 
            param = "metaclusters", 
            order = T, 
            title = "Phenograph Clustering on Normalized expression", 
            dim_red = "UMAP", facet_by_variable = FALSE)
