...
To use the `harmony` algorithm for correcting the principal components, we use the `harmonize_PCA()` and define the batch variable `batch_var`, in this example the experiment date 'exp'. If you want to use GPU support set argument GPU=T (only available in the respective docker image and supported hardware).
```{r, warning=FALSE}
