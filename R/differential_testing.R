...
#' @param anova_p.adjust.method p-value adjustment method to use for multiple test correction of Anova tests, e.g "bonferroni"(default) or "BH" (Benjamini-Hochberg). All available options can be checked in the documentation of the \link[rstatix]{adjust_pvalue} function from the package *rstatix*.
#' @param post_hoc logical, whether to perform post-hoc testing (TRUE, default) or not (FALSE).
#' @param post_hoc_p.adjust.method p-value adjustment method to use for post-hoc testing, e.g "bonferroni" (default). All available options can be checked in the documentation of the \link[rstatix]{adjust_pvalue} function from the package *rstatix*.
...
#' @details `frequency_anova_test()` is a wrapper function around \link[rstatix]{anova_test} and  \link[rstatix]{emmeans_test} implemented in the package *rstatix*. 
...
#' @param group_var string indicating variable in cell_anno that should be used to group samples in sample_var. group_var must have three or more levels.
#' @param sample_var string indicating variable in cell_anno that defines sample IDs to be used.
#' @param friedman_p.adjust.method p-value adjustment method to use for multiple comparisons of Friedman Rank Sum test, e.g "bonferroni" (default) or "BH" (Benjamini-Hochberg). All available options can be checked in the documentation of the \link[rstatix]{adjust_pvalue} function from the package *rstatix*.
#' @param post_hoc logical, whether to perform post-hoc testing (TRUE, default) or not (FALSE).
#' @param post_hoc_p.adjust.method p-value adjustment method to use for post-hoc testing, e.g "bonferroni" (default). All available options can be checked in the documentation of the \link[rstatix]{adjust_pvalue} function from the package *rstatix*.
...
#' @param numeric logical, if TRUE numeric levels in cluster_var are ordered in ascending order and "Cluster_" is pasted before number, if FALSE alphabetical ordering is applied.
#' @details `frequency_friedman_test()` is a wrapper function around \link[rstatix]{friedman_test},  \link[rstatix]{friedman_effsize} and  \link[rstatix]{wilcox_test} implemented in the package *rstatix*. The function first calculates cell population frequencies for each sample in sample_var. Then a Friedman Rank Sum test is performed for each cell population followed by p-value adjustment. If `post_hoc = T`, post-hoc testing with pairwise Wilcoxon Rank Sum Tests and p-value correction is performed for each significant Friedman Rank Sum test comparison.
#' @returns `frequency_friedman_test` returns a list of two data frames, "friedman_test" and "wilcox_test". "friedman_test" comprises results produced by \link[rstatix]{friedman_test} and \link[rstatix]{friedman_effsize} and "wilcox_test" contains results obtained by \link[rstatix]{wilcox_test}. Both data frames have one additional columns, "cluster", containing the information, which cell population was tested.
#' 
#' @export
...
#' @param group_var string indicating variable in cell_anno that should be used to group samples in sample_var. group_var must have three or more levels.
#' @param sample_var string indicating variable in cell_anno that defines sample IDs to be used.
#' @param kruskal_p.adjust.method p-value adjustment method to use for multiple comparisons of Kruskal-Wallis test, e.g "bonferroni" (default) or "BH" (Benjamini-Hochberg). All available options can be checked in the documentation of the \link[rstatix]{adjust_pvalue} function from the package *rstatix*.
#' @param post_hoc logical, whether to perform post-hoc testing (TRUE, default) or not (FALSE).
#' @param post_hoc_p.adjust.method p-value adjustment method to use for post-hoc testing, e.g "bonferroni" (default). All available options can be checked in the documentation of the \link[rstatix]{adjust_pvalue} function from the package *rstatix*.
...
#' @param var.equal logical, indicating whether variance in both groups should be treated as equal (TRUE) or not (FALSE, default). TRUE uses pooled variance, FALSE the Welch approximation, see documentation of \link[rstatix]{t_test} .
#' @param detailed logical if detailed output from \link[rstatix]{t_test} should be reported.
#' @param p.adjust.method p-value adjustment method to use for multiple comparison testing, e.g "bonferroni" (default) or "BH" (Benjamini-Hochberg). All available options can be checked in the documentation of the \link[rstatix]{adjust_pvalue} function from the package *rstatix*.
#' @param numeric logical, if TRUE numeric levels in cluster_var are ordered in ascending order and "Cluster_" is pasted before number, if FALSE alphabetical ordering is applied.
#' @import ?
...
#' @returns `frequency_t_test()` returns a data frame produced by \link[rstatix]{t_test} with two additional columns, "cluster" containing the information, which cell population was tested, and "applied_test", indicating which test was used.
#' 
#' @export
...
#' @param paired_test logical, indicating if a paired (TRUE) or unpaired test (FALSE, default) should be performed.
#' @param p.adjust.method p-value adjustment method to use for multiple comparison testing, e.g "bonferroni" (default) or "BH" (Benjamini-Hochberg). All available options can be checked in the documentation of the \link[rstatix]{adjust_pvalue} function from the package *rstatix*.
#' @param detailed logical if detailed output from \link[rstatix]{wilcox_test} should be reported.
#' @param numeric logical, if TRUE numeric levels in cluster_var are ordered in ascending order and "Cluster_" is pasted before number, if FALSE alphabetical ordering is applied.
#' @import ?
...
#' The test can either be run unpaired (two independent groups) or paired. Afterwards,  p-value adjustment will be performed across all comparisons that were made.
#' @returns `frequency_wilcox_test` returns a data frame produced by \link[rstatix]{wilcox_test} with two additional columns, "cluster" containing the information, which cell population was tested, and "applied_test", indicating which test was used.
#' 
#' @export
...
#' @import ?
#' @returns
#' A SummarizedExperiment object suitable to be used as input for *diffcyt* functions \link[diffcyt]{calcCounts}` and \link[diffcyt]{calcMedians}. The object contains the following components:
...
#' @param p.adjust.method p-value adjustment method to use for multiple comparison testing, e.g "BH" (Benjamini-Hochberg, default) or "bonferroni". All available options can be checked in the documentation of the \link[rstatix]{adjust_pvalue} function from the package *rstatix*
...
#' Wilcoxon Rank Sum Test is performed using the \link[stats]{wilcox.test} implemented in the *stats* package. Afterwards p-value adjustement is performed considering all comparisons that were made.
...
marker_wilcox_test<-function(fcd,
  ...
  cell_counts <- as.matrix(confusionMatrix(paste0(data$cluster), 
  ...
  data_stats <- data %>% group_by(group_var, cluster, .drop=T) %>% 
  ...
  #data_stats$mean_div   <- data_stats$mean1/data_stats$mean2
